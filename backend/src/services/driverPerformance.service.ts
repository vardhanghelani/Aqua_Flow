...
import { assertDriverInOrg, tenantFilter } from '../utils/tenant';
...
export async function computeDriverPerformance(
  ...
  const driver = await assertDriverInOrg(driverId, organizationId);
  ...
  const [deliveries, collections, settlements, damageLoss] = await Promise.all([
    Delivery.find({ ...orgMatch, driverId: driverOid, deliveryDate: dateRange }),
    DriverCollection.aggregate([
      { $match: { ...orgMatch, driverId: driverOid, collectionDate: dateRange } },
      { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } },
    ]),
    DriverDailySettlement.find({ ...orgMatch, driverId: driverOid, settlementDate: dateRange, status: 'approved' }),
